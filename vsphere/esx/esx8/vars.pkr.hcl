...
variable "vcenter_content_library_upload" {
    type        = bool
    description = "Upload the completed VM to a content library?"
    default     = false
}
...
variable "vcenter_export_template" {
    type        = bool
    description = "Export the VM as an OVF image?"
    default     = false
}
variable "vcenter_destroy" {
    type        = bool
    description = "Destroy the VM after the build completes?"
    default     = true
}
...
variable "build_ssh_timeout" {
    type        = string
    description = "Set the timeout for the SSH communicator to connect (e.g. '1h5m2s' or '2s')"
    default     = "5m"
}
